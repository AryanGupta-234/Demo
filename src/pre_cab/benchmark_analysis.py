...
def failure_buckets(predictions: Iterable[BenchmarkPrediction]) -> dict[str, Any]:
    ...
    return {
        ...
        # Historical benchmark terminology uses these code buckets to describe
        # the safety error represented by the opposite-direction prediction.
        "false_pass_finding_codes": dict(Counter(code for row in false_fail for code in row.finding_codes).most_common()),
        "false_fail_finding_codes": dict(Counter(code for row in false_pass for code in row.finding_codes).most_common()),
